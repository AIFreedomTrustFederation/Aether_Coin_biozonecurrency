...
import { userDapps, dappFiles, dappVersions } from '../../../shared/dapp-schema';
...
export class CodeGenerator {
  ...
  async generateFromPrompt(
    ...
    try {
      ...
      await nlpProcessor.saveConversation(
        userId,
        prompt,
        'user',
        requirements.intent,
      ...
      if (requirements.suggestedTemplate) {
        ...
        const template = await db.query.dappTemplates.findFirst({
          where: eq(db.query.dappTemplates.name, requirements.suggestedTemplate)
        });
        ...
        if (template) {
          ...
          await nlpProcessor.saveConversation(
            userId,
            `I've used the ${requirements.suggestedTemplate} template as a starting point. This template is designed for ${template.description}.`,
            'mysterion'
          );
        } else {
          ...
          await nlpProcessor.saveConversation(
            userId,
            `I couldn't find the suggested template, so I've created a custom implementation based on your requirements.`,
            'mysterion'
          );
        }
      } else {
        ...
        await nlpProcessor.saveConversation(
          userId,
          `I've created a custom implementation based on your requirements.`,
          'mysterion'
        );
      }
  ...
  private async generateFromTemplate(
    ...
    try {
      // Get the template
      const template = await db.query.dappTemplates.findFirst({
        where: eq(db.query.dappTemplates.id, templateId)
      });
      ...
      const parameters = this.extractParametersFromRequirements(
        requirements,
        template.parameters
      );
      ...
      const generatedCode = await schemaManager.applyParameters(
        template.schemaId,
        parameters
      );
      ...
      if (options.includeDocumentation) {
        result.docs = await schemaManager.generateDocumentation(
          template.schemaId,
          parameters
        );
      }
      ...
      if (options.securityChecks) {
        result.securityReport = await this.performSecurityChecks(
          generatedCode.code
        );
      }
   ...
   */
  private extractParametersFromRequirements(requirements: any, templateParams: any[]): Record<string, any> {
    ...
    templateParams.forEach((param: any) => {
      ...
      if (param.name === 'contractName') {
        ...
        const tokenEntity = requirements.entities.find((e: any) => e.entityType === 'Token');
        if (tokenEntity) {
        ...
        params[param.name] = requirements.constraints.some(
          (c: string) => c.toLowerCase().includes('pause')
        );
      } else if (param.name === 'isUpgradeable') {
        // Check if upgradeable was mentioned in constraints
        params[param.name] = requirements.constraints.some(
          (c: string) => c.toLowerCase().includes('upgrade')
        );
      }
    });
   ...
   */
  private formatContractName(name: string): string {
...
contract ${contractName} is Ownable {
   ...
   * @returns Test code
   */
  private generateBasicTests(contractName: string, requirements: any): string {
...
describe("${contractName}", function () {
   ...
   */
  private generateBasicUI(contractName: string, requirements: any): string {
...
## Entities
${requirements.entities.map((entity: any) => 
  `### ${entity.name} (${entity.entityType})
- **Properties**: ${entity.properties.join(', ')}
- **Relationships**: ${entity.relationships.length > 0 
    ? entity.relationships.map((rel: any) => `${rel.relation} ${rel.entity}`).join(', ') 
    : 'None'}`
).join('\n\n')}

## Actions
${requirements.actions.map((action: string) => `- ${action}`).join('\n')}
...
This contract uses the MIT License.`;
   ...
   */
  private async performSecurityChecks(code: string): Promise<SecurityReport> {
    ...
    const issues: { severity: 'low' | 'medium' | 'high' | 'critical'; description: string; location: string; recommendation: string }[] = [];
