...
class FractalCoinAPI {
  ...
  async getAccountInfo() {
    ...
    try {
      const response = await axios.get(`${this.apiEndpoint}/account/info`, {
  ...
  async getBalance() {
    ...
    try {
      const response = await axios.get(`${this.apiEndpoint}/account/balance`, {
        headers: this.getHeaders(),
      });
  ...
  async getTransactions(page: number = 1, limit: number = 20) {
    ...
    try {
      const response = await axios.get(`${this.apiEndpoint}/account/transactions`, {
        headers: this.getHeaders(),
        params: { page, limit },
      });
   ...
   */
  async transfer(to: string, amount: string, memo?: string) {
    if (this.mockMode) {
    ...
    try {
      const response = await axios.post(
        `${this.apiEndpoint}/transactions/transfer`,
        { to, amount, memo },
        { headers: this.getHeaders() }
      );
  ...
  async getStorageMetrics() {
    ...
    try {
      const response = await axios.get(`${this.apiEndpoint}/storage/metrics`, {
        headers: this.getHeaders(),
      });
   ...
   */
  async allocateStorage(bytes: number, options?: {
    purpose?: string;
    redundancy?: number;
    encryption?: string;
  }) {
  ...
  async listBridges() {
    ...
    try {
      const response = await axios.get(`${this.apiEndpoint}/bridges/list`, {
        headers: this.getHeaders(),
      });
   ...
   * @private
   */
  private mockTransactions(page: number, limit: number) {
    ...
    const transactions = Array.from({ length: actualCount }, (_, index) => {
      ...
      return {
        ...
        from: isEven ? '0x' + crypto.randomBytes(20).toString('hex') : this.mockAccountInfo().address,
        to: isEven ? this.mockAccountInfo().address : '0x' + crypto.randomBytes(20).toString('hex'),
   ...
   * @private
   */
  private mockTransfer(to: string, amount: string, memo?: string) {
   ...
   * @private
   */
  private mockAllocateStorage(bytes: number, options?: any) {
    ...
    const nodes = Array.from({ length: nodeCount }, () => ({
      ...
      endpoint: `https://storage-${crypto.randomBytes(8).toString('hex')}.fractalcoin.network`,
      status: 'online',
    }));
  ...
  private mockListBridges() {
    ...
    const bridges = Array.from({ length: totalBridges }, (_, index) => {
      ...
      return {
        ...
        cid: 'bafybei' + crypto.randomBytes(24).toString('hex'),
        createdAt,
        status: 'active',
        allocatedBytes,
        nodes,
   ...
   * @private
   */
  private mockRegisterBridge(bridgeData: { type: string; config: any }) {
    return {
      success: true,
      cid: 'bafybei' + crypto.randomBytes(24).toString('hex'),
...
// Singleton instance
let apiInstance: FractalCoinAPI | null = null;

/**
 * Get the FractalCoin API instance
 */
export function getFractalCoinAPI(): FractalCoinAPI {
  if (!apiInstance) {
