...
import { insertAgentTypeSchema, insertAgentInstanceSchema, insertAgentTaskSchema } from '../../shared/schema';
...
router.get('/types', async (req, res) => {
  try {
    ...
    const types = await storage.listAgentTypes(category);
    res.json(types);
  } catch (error) {
...
router.get('/types/:id', async (req, res) => {
  try {
    ...
    const agentType = await storage.getAgentType(id);
...
router.post('/types', async (req, res) => {
  try {
    const result = insertAgentTypeSchema.safeParse(req.body);
    
    if (!result.success) {
      return res.status(400).json({ error: result.error.format() });
    }
    
    const agentType = await storage.createAgentType(result.data);
...
router.get('/instances', async (req, res) => {
  try {
    ...
    const instances = await storage.listAgentInstances(status, owner);
    res.json(instances);
  } catch (error) {
...
router.get('/instances/system', async (req, res) => {
  try {
    ...
    const instances = await storage.listAgentInstances(undefined, 'system');
    res.json(instances);
  } catch (error) {
...
router.post('/instances', async (req, res) => {
  try {
    const result = insertAgentInstanceSchema.safeParse(req.body);
    
    if (!result.success) {
      return res.status(400).json({ error: result.error.format() });
    }
    
    const instance = await storage.createAgentInstance(result.data);
...
router.patch('/instances/:id/status', async (req, res) => {
  try {
    ...
    const updatedInstance = await storage.updateAgentInstance(id, { status });
...
router.patch('/instances/:id/config', async (req, res) => {
  try {
    ...
    const updatedInstance = await storage.updateAgentInstance(id, { configuration });
...
router.get('/instances/:id/tasks', async (req, res) => {
  try {
    ...
    const tasks = await storage.listAgentTasks(agentId, status);
    res.json(tasks);
  } catch (error) {
...
router.post('/instances/:id/tasks', async (req, res) => {
  try {
    ...
    const agent = await storage.getAgentInstance(agentId);
    ...
    const result = insertAgentTaskSchema.safeParse(taskData);
    
    if (!result.success) {
      return res.status(400).json({ error: result.error.format() });
    }
    
    const task = await storage.createAgentTask(result.data);
...
router.patch('/tasks/:id/status', async (req, res) => {
  try {
    ...
    const updatedTask = await storage.updateAgentTaskStatus(id, status, result);
...
router.post('/delegate', async (req, res) => {
  try {
    ...
    const agents = await storage.listAgentInstances('active', undefined);
    ...
    if (category) {
      ...
      const agentTypes = await storage.listAgentTypes();
