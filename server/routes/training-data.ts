...
import { insertTrainingDatasetSchema, insertTrainingDataFragmentSchema } from '../../shared/schema';
import { z } from 'zod';
...
router.get('/datasets', async (req, res) => {
  try {
    ...
    const datasets = await storage.listTrainingDatasets(dataType, status);
    res.json(datasets);
  } catch (error) {
...
router.post('/datasets', async (req, res) => {
  try {
    const result = insertTrainingDatasetSchema.safeParse(req.body);
    
    if (!result.success) {
      return res.status(400).json({ error: result.error.format() });
    }
    
    const dataset = await storage.createTrainingDataset(result.data);
...
router.patch('/datasets/:id', async (req, res) => {
  try {
    ...
    const updateSchema = insertTrainingDatasetSchema.partial();
    ...
    const updatedDataset = await storage.updateTrainingDataset(id, result.data);
...
router.post('/datasets/:id/store-filecoin', async (req, res) => {
  try {
    ...
    const updatedDataset = await storage.setFilecoinCid(id, cid);
...
router.get('/datasets/:id/fragments', async (req, res) => {
  try {
    ...
    const fragments = await storage.listTrainingDataFragments(datasetId);
    res.json(fragments);
  } catch (error) {
...
router.get('/fragments/:id', async (req, res) => {
  try {
    ...
    const fragment = await storage.getTrainingDataFragment(id);
...
router.post('/datasets/:id/fragments', async (req, res) => {
  try {
    ...
    const result = insertTrainingDataFragmentSchema.safeParse(fragmentData);
    
    if (!result.success) {
      return res.status(400).json({ error: result.error.format() });
    }
    
    const fragment = await storage.createTrainingDataFragment(result.data);
...
router.post('/fragments/:id/shard-fractal', async (req, res) => {
  try {
    ...
    const shardSchema = z.object({
      shardCount: z.number().int().min(1).max(64) // Maximum 64 shards per fragment
    });
    ...
    const shardIds = Array.from({ length: shardCount }, (_, i) => 
      `fractal-shard-${fragment.id}-${i}-${Math.random().toString(36).substring(2, 10)}`
    );
    
    // Update the fragment with the shard IDs
    // This is a simplification - in a real implementation, we would store these properly
    const updatedFragment = await storage.updateFragmentStorage(
      id,
      fragment.filecoinCid || '',
      shardIds
    );
...
router.post('/datasets/:id/preprocess', async (req, res) => {
  try {
    ...
    const optionsSchema = z.object({
      ...
      formatConversion: z.string().optional(),
