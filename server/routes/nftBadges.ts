...
import { insertNftBadgeSchema } from '@shared/schema';
...
router.get('/user/:userId', async (req, res) => {
  try {
    ...
    const badges = await storage.getNFTBadgesByUser(userId);
    res.json(badges);
  } catch (error) {
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const badge = await storage.getNFTBadge(id);
    if (!badge) {
...
router.post('/', validateRequest(insertNftBadgeSchema), async (req, res) => {
  try {
    // In a production app, we would check for admin permissions here
    const badge = await storage.createNFTBadge(req.body);
...
router.patch('/:id/mint', async (req, res) => {
  try {
    ...
    const updatedBadge = await storage.updateNFTBadge(id, {
      tokenId,
      mintedAt,
