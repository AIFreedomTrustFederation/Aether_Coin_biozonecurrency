...
class TransactionVerifier {
     ...
     */
    shouldHoldTransaction(scan) {
        // Hold transaction if:
        // 1. Contains any critical issues
        // 2. Contains 2+ high severity issues
        // 3. Risk score is over 75
        const hasCritical = scan.issues.some(issue => issue.severity === 'critical' && !issue.resolved);
        const highSeverityCount = scan.issues.filter(issue => issue.severity === 'high' && !issue.resolved).length;
     ...
     */
    getHoldReason(scan) {
        const criticalIssues = scan.issues.filter(issue => issue.severity === 'critical' && !issue.resolved);
        const highIssues = scan.issues.filter(issue => issue.severity === 'high' && !issue.resolved);
