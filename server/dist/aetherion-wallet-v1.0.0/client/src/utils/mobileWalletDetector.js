...
exports.detectProvidersFromWindow = detectProvidersFromWindow;
exports.detectMobileWallets = detectMobileWallets;
exports.openWallet = openWallet;
exports.getPreferredConnectionMethod = getPreferredConnectionMethod;
 ...
 */
async function isWalletInstalled(deepLink) {
    ...
    return new Promise((resolve) => {
        ...
        if (walletId === 'aetherion' && typeof window !== 'undefined') {
            ...
            Promise.resolve().then(() => __importStar(require('../core/blockchain/AetherionProvider'))).then(() => {
                if (window.aetherion) {
                    return resolve(true);
                }
 ...
 */
function detectProvidersFromWindow() {
    const providers = {};
    ...
    if (typeof window !== 'undefined') {
        ...
        if (window.ethereum) {
            ...
            providers.rainbow = !!(ethereum.isRainbow ||
                window.rainbow !== undefined ||
                ethereum._events?.accountsChanged?.some((fn) => fn.toString().includes('rainbow')) ||
                false);
            // Check for Aetherion integration with legacy Ethereum provider
            providers.aetherion = !!(ethereum.isAetherion ||
                ethereum.isATC ||
                ethereum._events?.accountsChanged?.some((fn) => fn.toString().includes('aetherion') || fn.toString().includes('atc')) ||
...
async function detectMobileWallets() {
    ...
    const detectedWalletIds = new Set();
    if (windowProviders.metamask)
        detectedWalletIds.add('metamask');
    if (windowProviders.trustwallet)
        detectedWalletIds.add('trust');
    if (windowProviders.coinbasewallet)
        detectedWalletIds.add('coinbase');
    if (windowProviders.rainbow)
        detectedWalletIds.add('rainbow');
    if (windowProviders.aetherion)
        detectedWalletIds.add('aetherion');
