...
const types_1 = require("../core/blockchain/types");
/**
...
function useWallet() {
    ...
    const [connectionStatus, setConnectionStatus] = (0, react_1.useState)(types_1.WalletConnectionStatus.DISCONNECTED);
    const [connectedAccounts, setConnectedAccounts] = (0, react_1.useState)([]);
    const [currentChainId, setCurrentChainId] = (0, react_1.useState)('0xa37'); // Default to Aetherion mainnet
    const [currentBalance, setCurrentBalance] = (0, react_1.useState)('0');
    // Get user wallets
    const { data: wallets = [], isLoading: isLoadingWallets, error: walletsError, } = (0, react_query_1.useQuery)({
        queryKey: ['/api/wallets'],
        queryFn: () => (0, queryClient_1.apiRequest)('/api/wallets')
    });
    // Create a new wallet
    const createWalletMutation = (0, react_query_1.useMutation)({
        mutationFn: (data) => (0, queryClient_1.apiRequest)('/api/wallets', 'POST', data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['/api/wallets'] });
        }
    });
    // Set default wallet
    const setDefaultWalletMutation = (0, react_query_1.useMutation)({
        mutationFn: (walletId) => (0, queryClient_1.apiRequest)(`/api/wallets/${walletId}/default`, 'POST'),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['/api/wallets'] });
        }
    });
    ...
    const connectWallet = (0, react_1.useCallback)(async () => {
        try {
            ...
            setConnectionStatus(types_1.WalletConnectionStatus.CONNECTING);
            ...
            setConnectionStatus(types_1.WalletConnectionStatus.CONNECTED);
            ...
            setConnectionStatus(types_1.WalletConnectionStatus.ERROR);
            throw error;
        }
    }, [wallets, createWalletMutation]);
    // Disconnect from blockchain wallet
    const disconnectWallet = (0, react_1.useCallback)(async () => {
        // Update connection status
        setConnectionStatus(types_1.WalletConnectionStatus.DISCONNECTED);
    ...
    (0, react_1.useEffect)(() => {
        if (connectionStatus === types_1.WalletConnectionStatus.CONNECTED && connectedAccounts.length > 0) {
