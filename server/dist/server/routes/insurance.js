...
router.get('/api/laos/:laoId/risk-pools', async (req, res) => {
    try {
        ...
        const riskPools = await storage_1.storage.getRiskPoolsByLAO(laoId);
        res.json(riskPools);
    }
    catch (error) {
...
router.get('/api/risk-pools/by-type/:type', async (req, res) => {
    try {
        ...
        const riskPools = await storage_1.storage.getRiskPoolsByType(policyType);
        res.json(riskPools);
    }
    catch (error) {
...
router.post('/api/laos/:laoId/risk-pools', isAuthenticated, async (req, res) => {
    try {
        ...
        const member = await storage_1.storage.getLAOMemberByUserAndLAO(req.session.userId, laoId);
        if (!member || (member.role !== 'owner' && member.role !== 'manager')) {
            return res.status(403).json({ error: 'Forbidden: Only LAO owners and managers can create risk pools' });
        }
        const validatedData = schema_1.insertInsuranceRiskPoolSchema.parse({
            ...req.body,
            laoId,
            createdAt: new Date(),
            updatedAt: new Date()
        });
        const newRiskPool = await storage_1.storage.createRiskPool(validatedData);
...
router.get('/api/risk-pools/:id/balance', async (req, res) => {
    try {
        ...
        const balance = await storage_1.storage.getRiskPoolBalance(id);
        res.json({ balance });
    }
    catch (error) {
...
router.post('/api/risk-pools/:id/balance', isAuthenticated, async (req, res) => {
    try {
        ...
        const riskPool = await storage_1.storage.getRiskPool(id);
        if (!riskPool) {
            return res.status(404).json({ error: 'Risk pool not found' });
        }
        // Check if user is owner or manager of this LAO
        const member = await storage_1.storage.getLAOMemberByUserAndLAO(req.session.userId, riskPool.laoId);
        if (!member || (member.role !== 'owner' && member.role !== 'manager')) {
            return res.status(403).json({ error: 'Forbidden: Only LAO owners and managers can update risk pool balance' });
        }
        const updatedRiskPool = await storage_1.storage.updateRiskPoolBalance(id, amount, isDeposit);
        if (!updatedRiskPool) {
            return res.status(404).json({ error: 'Risk pool not found' });
        }
...
router.get('/api/members/:memberId/policies', async (req, res) => {
    try {
        ...
        const policies = await storage_1.storage.getPoliciesByHolder(memberId);
        res.json(policies);
    }
    catch (error) {
...
router.get('/api/risk-pools/:id/policies', async (req, res) => {
    try {
        ...
        const policies = await storage_1.storage.getPoliciesByRiskPool(riskPoolId);
        res.json(policies);
    }
    catch (error) {
...
router.post('/api/policies', isAuthenticated, async (req, res) => {
    try {
        ...
        const member = await storage_1.storage.getLAOMember(policyHolderId);
        if (!member) {
        ...
        const userMembership = await storage_1.storage.getLAOMemberByUserAndLAO(req.session.userId, member.laoId);
        ...
        const riskPool = await storage_1.storage.getRiskPool(riskPoolId);
        if (!riskPool) {
            return res.status(404).json({ error: 'Risk pool not found' });
        }
        ...
        const validatedData = schema_1.insertInsurancePolicySchema.parse({
            ...req.body,
            policyNumber,
            status: 'pending',
            createdAt: new Date(),
        ...
        const newPolicy = await storage_1.storage.createPolicy(validatedData);
        // Create initial premium
        await storage_1.storage.createPremium({
...
router.post('/api/policies/:id/activate', isAuthenticated, async (req, res) => {
    try {
        ...
        const policy = await storage_1.storage.getPolicy(id);
        if (!policy) {
            return res.status(404).json({ error: 'Policy not found' });
        }
        // Check if user is owner or manager of the LAO
        const riskPool = await storage_1.storage.getRiskPool(policy.riskPoolId);
        const member = await storage_1.storage.getLAOMemberByUserAndLAO(req.session.userId, riskPool.laoId);
        if (!member || (member.role !== 'owner' && member.role !== 'manager')) {
            return res.status(403).json({ error: 'Forbidden: Only LAO owners and managers can activate policies' });
        }
        const activatedPolicy = await storage_1.storage.activatePolicy(id, new Date(startDate), new Date(endDate));
...
router.get('/api/policies/:id/claims', async (req, res) => {
    try {
        ...
        const claims = await storage_1.storage.getClaimsByPolicy(policyId);
        res.json(claims);
    }
    catch (error) {
...
router.post('/api/policies/:id/claims', isAuthenticated, async (req, res) => {
    try {
        const policyId = parseInt(req.params.id);
        const policy = await storage_1.storage.getPolicy(policyId);
        if (!policy) {
            return res.status(404).json({ error: 'Policy not found' });
        }
        ...
        // Check if the authenticated user is the policy holder or has appropriate permissions
        const policyHolder = await storage_1.storage.getLAOMember(policy.policyHolderId);
        ...
        const validatedData = schema_1.insertInsuranceClaimSchema.parse({
            ...req.body,
            policyId,
        ...
        const newClaim = await storage_1.storage.createClaim(validatedData);
...
router.post('/api/claims/:id/review', isAuthenticated, async (req, res) => {
    try {
        ...
        const claim = await storage_1.storage.getClaim(id);
        if (!claim) {
        ...
        const policy = await storage_1.storage.getPolicy(claim.policyId);
        const riskPool = await storage_1.storage.getRiskPool(policy.riskPoolId);
        // Check if user is owner or manager of the LAO
        const reviewer = await storage_1.storage.getLAOMemberByUserAndLAO(req.session.userId, riskPool.laoId);
        ...
        const reviewedClaim = await storage_1.storage.reviewClaim(id, reviewer.id, approved, approvedAmount, notes);
...
router.get('/api/policies/:id/premiums', async (req, res) => {
    try {
        const policyId = parseInt(req.params.id);
        const premiums = await storage_1.storage.getPremiumsByPolicy(policyId);
        res.json(premiums);
    }
    catch (error) {
...
router.post('/api/premiums/:id/pay', isAuthenticated, async (req, res) => {
    try {
        ...
        const premium = await storage_1.storage.getPremium(id);
        if (!premium) {
            return res.status(404).json({ error: 'Premium not found' });
        }
        // Get policy and check if user is the policy holder
        const policy = await storage_1.storage.getPolicy(premium.policyId);
        const policyHolder = await storage_1.storage.getLAOMember(policy.policyHolderId);
        ...
        const paidPremium = await storage_1.storage.recordPremiumPayment(id, transactionHash);
        // Update risk pool balance
        const riskPool = await storage_1.storage.getRiskPool(policy.riskPoolId);
        await storage_1.storage.updateRiskPoolBalance(riskPool.id, Number(premium.amount), true);
...
router.get('/api/members/:memberId/iul-policies', async (req, res) => {
    try {
        const memberId = parseInt(req.params.memberId);
        const policies = await storage_1.storage.getIULPoliciesByMember(memberId);
        res.json(policies);
    }
    catch (error) {
...
router.post('/api/iul-policies', isAuthenticated, async (req, res) => {
    try {
        ...
        const validatedData = schema_1.insertIULPolicySchema.parse({
            ...req.body,
            policyNumber,
            status: 'active',
            issueDate: new Date(),
        ...
        const newPolicy = await storage_1.storage.createIULPolicy(validatedData);
        // Record initial premium payment transaction
        await storage_1.storage.recordIULPolicyTransaction({
            policyId: newPolicy.id,
...
router.get('/api/iul-policies/:id/transactions', async (req, res) => {
    try {
        const policyId = parseInt(req.params.id);
        const transactions = await storage_1.storage.getIULPolicyTransactions(policyId);
...
router.post('/api/iul-policies/:id/index', isAuthenticated, async (req, res) => {
    try {
        ...
        const policy = await storage_1.storage.getIULPolicy(id);
        if (!policy) {
            return res.status(404).json({ error: 'IUL policy not found' });
        }
        // Check if user is owner or manager of the LAO
        const member = await storage_1.storage.getLAOMember(policy.memberId);
        const laoMember = await storage_1.storage.getLAOMemberByUserAndLAO(req.session.userId, member.laoId);
        ...
        const indexedPolicy = await storage_1.storage.indexIULPolicy(id, indexRate);
