...
exports.getUserContributionPoints = exports.generateResponse = void 0;
const openai_1 = require("openai");
const quantum_vault_1 = require("./quantum-vault");
 ...
 */
const getClientKey = (apiKey) => {
    return (0, crypto_1.createHash)('sha256').update(apiKey).digest('hex').substring(0, 16);
};
/**
...
const getOpenAIClient = (apiKey) => {
    ...
    if (!openaiClients[clientKey]) {
        openaiClients[clientKey] = new openai_1.OpenAI({
            apiKey,
        });
    }
 ...
 */
const selectNextApiKey = async (userId) => {
    try {
        // First try to get user's own keys
        const userKeys = await storage_1.storage.getUserApiKeysByService(userId, 'openai');
        ...
        if (activeKeys.length > 0) {
            ...
            if (selectedKey.vaultKeyId) {
                const { apiKey, isSuccess } = await (0, quantum_vault_1.retrieveApiKey)(userId, selectedKey.vaultKeyId);
                if (isSuccess && apiKey) {
 ...
 */
const logApiKeyUsage = async (keyId, userId, contributorId, promptTokens, completionTokens, totalTokens) => {
    try {
        // First increment the usage count
        await storage_1.storage.incrementUserApiKeyUsage(keyId);
        ...
        const trainingData = await storage_1.storage.createMysterionTrainingData({
            apiKeyId: keyId,
            userId,
            contributorId,
            promptTokens,
...
const generateResponse = async (messages, options) => {
    try {
        ...
        const points = await logApiKeyUsage(keyRecord.id, options.userId, contributorId, usage.promptTokens, usage.completionTokens, usage.totalTokens);
        return {
            content,
            usage,
            success: true,
 ...
 */
const getUserContributionPoints = async (userId) => {
    try {
        ...
        for (const key of userKeys) {
            const trainingRecords = await storage_1.storage.getMysterionTrainingDataByApiKeyId(key.id);
...
exports.getUserContributionPoints = getUserContributionPoints;
