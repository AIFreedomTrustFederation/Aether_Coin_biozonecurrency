...
const scroll_keeper_routes_1 = __importDefault(require("./routes/scroll-keeper-routes"));
...
const lao_1 = __importDefault(require("./routes/lao"));
...
async function registerRoutes(app) {
    ...
    app.use('/api/scrollkeeper', scroll_keeper_routes_1.default);
    ...
    app.use('/api', lao_1.default);
    // Mount Insurance System routes
    app.use('/api', insurance_1.default);
...
FractalCoin represents not just a technological innovation but a fundamental reimagining of how value can be created and distributed in a digital economy. By aligning blockchain incentives with natural mathematical patterns, we create a system that is inherently fair, sustainable, and resistant to the centralization pressures that have affected previous cryptocurrency implementations.`);
    ...
    app.get("/api/transactions/recent", async (req, res) => {
        try {
            ...
            const transactions = await storage_1.storage.getRecentTransactions(userId, limit);
    ...
    app.post("/api/transactions", async (req, res) => {
        try {
            ...
            const wallet = await storage_1.storage.getWallet(transaction.walletId);
            if (wallet) {
    ...
    app.get("/api/transactions/layer2", async (req, res) => {
        try {
            ...
            const transactions = await storage_1.storage.getLayer2Transactions(userId, layer2Type);
            res.json(transactions);
        }
        catch (error) {
    ...
    app.patch("/api/transactions/:id/description", async (req, res) => {
        try {
            ...
            const transaction = await storage_1.storage.updateTransactionDescription(id, description);
            if (!transaction) {
    ...
    app.patch("/api/transactions/:id/layer2", async (req, res) => {
        try {
            ...
            const transaction = await storage_1.storage.updateTransactionLayer2Info(id, isLayer2, layer2Type, layer2Data);
            if (!transaction) {
                return res.status(404).json({ message: "Transaction not found" });
            }
            res.json(transaction);
    ...
    app.get("/api/contracts", async (req, res) => {
        try {
            const userId = 1;
            const contracts = await storage_1.storage.getSmartContractsByUserId(userId);
            res.json(contracts);
        }
        catch (error) {
    ...
    app.post("/api/contracts", async (req, res) => {
        try {
            ...
            const contract = await storage_1.storage.createSmartContract(contractData);
    ...
    app.patch("/api/contracts/:id/status", async (req, res) => {
        try {
            ...
            const updatedContract = await storage_1.storage.updateSmartContractStatus(id, status);
            if (!updatedContract) {
    ...
    app.get("/api/ai/logs", async (req, res) => {
        try {
            ...
            const logs = await storage_1.storage.getAiMonitoringLogs(userId, limit);
            res.json(logs);
        }
        catch (error) {
    ...
    app.post("/api/ai/logs", async (req, res) => {
        try {
            ...
            const log = await storage_1.storage.createAiMonitoringLog(logData);
    ...
    app.get("/api/cids", async (req, res) => {
        try {
            const userId = 1;
            const entries = await storage_1.storage.getCidEntriesByUserId(userId);
            res.json(entries);
        }
        catch (error) {
    ...
    app.post("/api/cids", async (req, res) => {
        try {
            ...
            const entry = await storage_1.storage.createCidEntry(entryData);
    ...
    app.patch("/api/cids/:id/status", async (req, res) => {
        try {
            ...
            const updatedEntry = await storage_1.storage.updateCidEntryStatus(id, status);
            if (!updatedEntry) {
    ...
    app.get("/api/payment-methods", async (req, res) => {
        try {
            ...
            const paymentMethods = await storage_1.storage.getPaymentMethodsByUserId(userId);
    ...
    app.patch("/api/payment-methods/:id/default", async (req, res) => {
        try {
            ...
            const updatedMethod = await storage_1.storage.updatePaymentMethodDefault(id, isDefault);
            if (!updatedMethod) {
    ...
    app.delete("/api/payment-methods/:id", async (req, res) => {
        try {
            ...
            const success = await storage_1.storage.deletePaymentMethod(id);
            if (!success) {
                return res.status(404).json({ message: "Payment method not found" });
            }
            res.status(204).end();
    ...
    app.get("/api/payments", async (req, res) => {
        try {
            const userId = 1; // For demo purposes
            const payments = await storage_1.storage.getPaymentsByUserId(userId);
            res.json(payments);
        }
        catch (error) {
    ...
    app.get("/api/wallet-health/scores", async (req, res) => {
        try {
            const userId = 1; // For demo purposes
            const scores = await storage_1.storage.getWalletHealthScoresByUserId(userId);
            res.json(scores);
        }
        catch (error) {
    ...
    app.get("/api/wallet-health/scores/wallet/:walletId", async (req, res) => {
        try {
            ...
            const score = await storage_1.storage.getWalletHealthScoreByWalletId(walletId);
            if (!score) {
    ...
    app.get("/api/wallet-health/issues/:scoreId", async (req, res) => {
        try {
            ...
            const issues = await storage_1.storage.getWalletHealthIssuesByScoreId(scoreId);
            res.json(issues);
        }
        catch (error) {
    ...
    app.post("/api/wallet-health/scores", async (req, res) => {
        try {
            ...
            const score = await storage_1.storage.createWalletHealthScore(scoreData);
    ...
    app.post("/api/wallet-health/issues", async (req, res) => {
        try {
            ...
            const issue = await storage_1.storage.createWalletHealthIssue(issueData);
    ...
    app.patch("/api/wallet-health/issues/:id/resolved", async (req, res) => {
        try {
            ...
            const updatedIssue = await storage_1.storage.updateWalletHealthIssueResolved(id, resolved);
            if (!updatedIssue) {
    ...
    app.get("/api/notification-preferences", async (req, res) => {
        try {
            const userId = 1; // For demo purposes
            const preferences = await storage_1.storage.getNotificationPreferenceByUserId(userId);
            if (!preferences) {
    ...
    app.patch("/api/notification-preferences", async (req, res) => {
        try {
            ...
            const updatedPreferences = await storage_1.storage.updateNotificationPreferences(userId, updatedData);
    ...
    app.post("/api/notification-preferences/phone", async (req, res) => {
        try {
            ...
            if (!twilioService.isTwilioConfigured()) {
                ...
                await storage_1.storage.updatePhoneNumber(userId, phoneNumber, true);
                return res.json({
                    success: true,
    ...
    app.post("/api/notification-preferences/verify", async (req, res) => {
        try {
            ...
            if (!twilioService.isTwilioConfigured()) {
                ...
                await storage_1.storage.verifyPhoneNumber(userId, true);
                return res.json({
                    success: true,
    ...
    app.post("/api/notification-preferences/matrix", async (req, res) => {
        try {
            ...
            if (!isMatrixConfigured) {
                await storage_1.storage.updateMatrixId(userId, matrixId, true);
                return res.json({
                    success: true,
            ...
            try {
                ...
                await storage_1.storage.updateMatrixId(userId, matrixId, false);
                // For the demo, we'll mark as verified immediately
                // In a real app, we'd have a verification flow similar to SMS
                await storage_1.storage.verifyMatrixId(userId, true);
                res.json({
                    success: true,
