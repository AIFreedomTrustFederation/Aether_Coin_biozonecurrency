...
router.get("/", async (req, res) => {
    try {
        ...
        const userId = 1;
        const keys = await api_key_service_1.apiKeyService.getApiKeysByUserId(userId);
 ...
 */
router.post("/", requireAuth, admin_auth_1.checkAdminPrivilege, async (req, res) => {
    try {
        const authReq = req;
        // Validate incoming data
        const keyData = api_key_schema_1.insertApiKeySchema
            .extend({
        ...
        const apiKey = await api_key_service_1.apiKeyService.createApiKey({
            ...keyData,
            userId
        });
...
router.get("/:id", async (req, res) => {
    try {
        ...
        const userId = 1;
        const key = await api_key_service_1.apiKeyService.getApiKeyById(id);
        if (!key) {
...
router.patch("/:id/revoke", async (req, res) => {
    try {
        ...
        const revokedKey = await api_key_service_1.apiKeyService.revokeApiKey(id);
        // Don't send the actual key in the response
...
router.get("/:id/usage", async (req, res) => {
    try {
        ...
        const usage = await api_key_service_1.apiKeyService.getApiKeyUsage(id);
        res.json(usage);
    }
    catch (error) {
...
router.get("/:id/connections", async (req, res) => {
    try {
        ...
        const connections = await api_key_service_1.apiKeyService.getApiKeyConnections(id);
...
router.delete("/:id/connections/:connectionId", async (req, res) => {
    try {
        ...
        const success = await api_key_service_1.apiKeyService.terminateConnection(connectionId);
        if (!success) {
...
const validateApiKey = async (req, res, next) => {
    try {
        ...
        const keyDetails = await api_key_service_1.apiKeyService.validateApiKey(apiKey);
        if (!keyDetails) {
        ...
        // Track API key usage
        await api_key_service_1.apiKeyService.trackApiKeyUsage(keyDetails.id, req.path, req.ip);
        next();
    }
    catch (error) {
