 ...
 */
const security_analysis_service_1 = require("../security/security-analysis-service");
/**
...
class CompletionService {
     ...
     */
    getCompletionSuggestions(code, position, prefix) {
        const suggestions = [];
     ...
     */
    getCompletionDocumentation(item) {
        // Look for the item in security patterns
        const securityPattern = SECURITY_PATTERNS.find(p => p.pattern === item || p.example.includes(item));
        if (securityPattern) {
     ...
     */
    getSecurePatterns(vulnerabilities) {
        const patterns = {};
    ...
    async suggestCodeImprovements(code) {
        ...
        const analysis = await security_analysis_service_1.securityAnalysisService.analyzeContract(code);
     ...
     */
    addContextualSuggestions(suggestions, scope, prefix, currentLine, prevLine) {
     ...
     */
    addContractPatternSuggestions(suggestions, scope, code) {
...
exports.completionService = new CompletionService();
