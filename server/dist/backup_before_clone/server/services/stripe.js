...
exports.stripeService = {
     ...
     */
    async createPaymentIntent(amount, currency, description, userId, walletId, metadata = {}) {
        try {
            if (!stripe) {
            ...
            await storage_1.storage.createPayment(paymentData);
            return {
     ...
     */
    async savePaymentMethod(userId, stripePaymentMethodId, isDefault = false) {
        try {
            if (!stripe) {
                throw new Error('Stripe is not initialized');
            }
 ...
 */
async function updatePaymentStatus(providerPaymentId, status) {
    try {
        // Find the payment by provider payment ID
        const payments = await storage_1.storage.getPaymentsByProviderPaymentId(providerPaymentId);
        if (!payments || payments.length === 0) {
            return null;
        }
        const payment = payments[0];
        // Update the status
        const updatedPayment = await storage_1.storage.updatePaymentStatus(payment.id, status, status === 'completed' || status === 'failed' ? new Date() : undefined);
        // If payment is completed and has a wallet ID, update wallet balance
        if (status === 'completed' && payment.walletId) {
            const wallet = await storage_1.storage.getWallet(payment.walletId);
            if (wallet) {
                const amount = parseFloat(payment.amount);
                const currentBalance = parseFloat(wallet.balance);
                const newBalance = currentBalance + (amount / 100); // amount is in cents
                await storage_1.storage.updateWalletBalance(payment.walletId, newBalance.toString());
            }
        }
