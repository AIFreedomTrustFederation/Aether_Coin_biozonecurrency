...
class EscrowService {
    constructor() {
        this.apiEndpoint = '/api/escrow';
        this.isInitialized = false;
        this.defaultHoldingPeriod = 24; // Default: 24 hours
     ...
     */
    initialize(apiEndpoint, defaultHoldingPeriod) {
        if (apiEndpoint) {
     ...
     */
    async createEscrow(senderAddress, recipientAddress, amount, tokenSymbol, network, holdingPeriod, conditions) {
        ...
        try {
            ...
            if (conditions && conditions.length > 0) {
                escrowConditions.push(...conditions.map(condition => ({
                    ...condition,
                    id: (0, uuid_1.v4)(),
                    isMet: false
                })));
     ...
     */
    getEscrowsByAddress(address, role = 'both') {
     ...
     */
    async fundEscrow(escrowId, senderAddress) {
     ...
     * @returns Promise resolving to the updated escrow transaction
     */
    async approveEscrow(escrowId, approverAddress) {
        const escrow = this.getEscrow(escrowId);
        if (!escrow) {
            throw new Error('Escrow transaction not found');
        }
     ...
     * @returns Promise resolving to the updated escrow transaction
     */
    async cancelEscrow(escrowId, cancellerAddress) {
        const escrow = this.getEscrow(escrowId);
        if (!escrow) {
            throw new Error('Escrow transaction not found');
        }
     ...
     */
    async createDispute(escrowId, disputerAddress, reason, details) {
        const escrow = this.getEscrow(escrowId);
        if (!escrow) {
            throw new Error('Escrow transaction not found');
        }
     ...
     */
    async resolveDispute(disputeId, outcome, reason, splitRatio, resolverAddress) {
     ...
     */
    updateDefaultHoldingPeriod(hours) {
        if (hours > 0) {
...
exports.default = new EscrowService();
