 ...
 */
export function useNodeMarketplaceApi() {
  ...
  useEffect(() => {
    const fetchMarketplaceData = async () => {
      try {
        ...
        const client = apiClientFactory.getClient("nodeMarketplace");
  ...
  const deployService = async (serviceData: any) => {
    try {
      setIsLoading(true);
  ...
  const calculateRewards = (resources: {
    ...
    const fractalcoinDaily = 
      (resources.cpu ? resources.cpu * 0.3 : 0) + 
