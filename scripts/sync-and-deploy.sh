...
#   --deploy: Automatically run the deploy-to-cpanel.sh script after synchronization
...
check_for_changes() {
  ...
  if [ "$has_changes" = false ]; then
    ...
    # If deploy flag is set, proceed with deployment even if no changes
    if [ "$DEPLOY_AFTER_SYNC" = true ]; then
      log "No changes detected, but --deploy flag is set. Proceeding with deployment." "INFO"
      return 0
    else
      exit 0
    fi
...
# Function to deploy to cPanel
deploy_to_cpanel() {
  if [ -f "${TARGET_REPO}/deploy-to-cpanel.sh" ]; then
    log "Starting deployment to cPanel..." "INFO"
    
    # Change to target repository directory
    cd "${TARGET_REPO}"
    
    # Make deploy script executable if it's not already
    ...
    if [ $? -eq 0 ]; then
      log "Deployment to cPanel completed successfully" "INFO"
      send_slack_notification "success" "Deployment to cPanel completed successfully"
      return 0
    else
      log "Deployment to cPanel failed" "ERROR"
      send_slack_notification "error" "Deployment to cPanel failed. Check logs for details."
      return 1
    fi
  else
    log "deploy-to-cpanel.sh script not found in target repository" "ERROR"
    send_slack_notification "error" "deploy-to-cpanel.sh script not found in target repository"
    return 1
  fi
}
...
main() {
  ...
  # Deploy to cPanel if requested
  if [ "$DEPLOY_AFTER_SYNC" = true ]; then
    log "=== cPanel Deployment Started ===" "INFO"
    deploy_to_cpanel
    log "=== cPanel Deployment Completed ===" "INFO"
  fi
}
