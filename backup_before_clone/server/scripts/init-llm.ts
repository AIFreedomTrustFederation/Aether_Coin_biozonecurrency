...
import { llmPrompts } from "../../shared/schema-proxy";
...
async function initializeLlmService() {
  ...
  try {
    ...
    const isConnected = await llmService.initialize();
...
async function addDefaultPrompts() {
  ...
  const existingPrompts = await db.select({ name: llmPrompts.name }).from(llmPrompts);
