...
// Declare the extension of the IStorage interface with additional methods
declare module './storage' {
  export interface IStorage {
    ...
    getTransactionsByWalletId(walletId: number, limit?: number): Promise<any[]>;
    createTransaction(data: any): Promise<any>;
    updateTransactionDescription(id: number, description: string): Promise<any>;
    updateTransactionLayer2Info(id: number, layer2Info: any): Promise<any>;
    getLayer2Transactions(userId: number, limit?: number): Promise<any[]>;
    ...
    getPaymentsByUserId(userId: number, limit?: number): Promise<any[]>;
    ...
    updateWalletHealthIssueResolved(id: number, isResolved: boolean): Promise<any>;
    ...
    verifyPhoneNumber(userId: number, code: string): Promise<any>;
    updateMatrixId(userId: number, matrixId: string): Promise<any>;
    verifyMatrixId(userId: number, code: string): Promise<any>;
  }
}

// Add dynamic methods to the storage object
// Using type assertion to avoid TypeScript errors
const extendedStorage = storage as any;

// Wallet methods
extendedStorage.getWalletsByUserId = async (userId: number) => {
...
extendedStorage.getWallet = async (id: number) => {
...
// Transaction methods
extendedStorage.getRecentTransactions = async (userId: number, limit = 5) => {
...
extendedStorage.getTransactionsByWalletId = async (walletId: number, limit = 10) => {
...
extendedStorage.createTransaction = async (data: any) => {
...
extendedStorage.updateTransactionDescription = async (id: number, description: string) => {
  console.log(`Updating transaction ${id} description to ${description} - Extended method`);
  return { id, description, updated: true };
};

extendedStorage.updateTransactionLayer2Info = async (id: number, layer2Info: any) => {
  console.log(`Updating transaction ${id} Layer2 info - Extended method`);
  return { id, layer2Info, updated: true };
};

extendedStorage.getLayer2Transactions = async (userId: number, limit = 5) => {
...
// AI Monitoring methods
extendedStorage.getAiMonitoringLogs = async (userId: number, limit = 10) => {
...
extendedStorage.createAiMonitoringLog = async (data: any) => {
...
// Wallet health methods
extendedStorage.getWalletHealthScoresByUserId = async (userId: number) => {
...
extendedStorage.getWalletHealthScoreByWalletId = async (walletId: number) => {
...
extendedStorage.getWalletHealthIssuesByScoreId = async (scoreId: number) => {
...
extendedStorage.createWalletHealthScore = async (data: any) => {
...
extendedStorage.createWalletHealthIssue = async (data: any) => {
...
extendedStorage.updateWalletHealthIssueResolved = async (id: number, isResolved: boolean) => {
...
// Notification methods
extendedStorage.getNotificationPreferenceByUserId = async (userId: number) => {
...
extendedStorage.updateNotificationPreferences = async (userId: number, preferences: any) => {
...
extendedStorage.updatePhoneNumber = async (userId: number, phoneNumber: string) => {
...
extendedStorage.verifyPhoneNumber = async (userId: number, code: string) => {
...
extendedStorage.updateMatrixId = async (userId: number, matrixId: string) => {
...
extendedStorage.verifyMatrixId = async (userId: number, code: string) => {
...
extendedStorage.paymentProvider.checkPaymentStatus = async (paymentId: string) => {
...
extendedStorage.paymentProvider.handleWebhookEvent = async (event: any) => {
