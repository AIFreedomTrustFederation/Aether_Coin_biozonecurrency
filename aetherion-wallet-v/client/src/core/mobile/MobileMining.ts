...
export type MiningStatus = 'idle' | 'starting' | 'mining' | 'paused' | 'stopping' | 'error';
export type MiningAlgorithm = 'quantum-resistant' | 'standard' | 'eco';
export type BatteryStatus = 'critical' | 'low' | 'normal' | 'charging' | 'unknown';

export interface MiningOptions {
  algorithm: MiningAlgorithm;
  threads: number;
  throttle: number; // 0-100, percentage of CPU to use
  maxBatteryUsage: number; // 0-100, percentage of battery allowed to use
  pauseOnBatteryLow: boolean;
  pauseOnOverheat: boolean;
  mineOnlyWhenCharging: boolean;
  maxTemperature: number; // in Celsius
  minHashRate: number; // minimum acceptable hash rate
}
 ...
 */
export class MobileMining {
  private static instance: MobileMining;
   ...
   */
  public static getInstance(): MobileMining {
  ...
  private updateBatteryStatus(): void {
    ...
    if (this.stats.status === 'paused') {
      const canResume = (
        (!this.options.pauseOnBatteryLow || this.stats.batteryStatus !== 'critical') &&
   ...
   */
  public pauseMining(reason?: string): boolean {
    if (this.stats.status !== 'mining') {
      return false;
    }
   ...
   */
  public updateOptions(options: Partial<MiningOptions>): boolean {
    try {
      // Update options
      this.options = {
        ...this.options,
   ...
   */
  public getOptions(): MiningOptions {
    return { ...this.options };
  }
   ...
   */
  public getAvailableAlgorithms(): MiningAlgorithm[] {
   ...
   */
  public getAlgorithmStats(algorithm: MiningAlgorithm): {
    name: string;
    description: string;
    efficiency: number; // 0-100
    reward: number; // relative reward rate
    difficulty: number; // relative difficulty
    powerUsage: number; // relative power usage
  } | null {
    switch (algorithm) {
   ...
   */
  public getOptimalSettings(): MiningOptions {
...
export default MobileMining.getInstance();